...
(function() {
    ...
    function insertUpdateAllButton() {
        if (document.querySelector(".btn-wrap.torn-bazaar-update-all-prices")) return;
    ...
    function findUpdateAllButtonTarget(firstUpdateButton) {
        let saveButton = Array.from(document.querySelectorAll('input, button'))
            .find(element => /^save changes$/i.test((element.value || element.textContent || '').trim()));

        if (saveButton && saveButton.parentElement) {
            return saveButton.parentElement.parentElement || saveButton.parentElement;
        }

        let manageHeader = Array.from(document.querySelectorAll('div, h4, h5'))
        ...
        return firstUpdateButton.closest('div[class*="row___"], div[data-testid="sortable-item"], li.clearfix')?.parentElement || document.querySelector(".content-wrapper") || document.body;
    }

    function getManageUpdateButtons() {
        return Array.from(document.querySelectorAll('span.torn-bazaar-fill-qty-price input[value="Update"]'))
    ...
    async function updateAllPrices(button) {
        ...
        try {
            for (let i = 0; i < updateButtons.length; i++) {
                ...
                if (!updateButton.isConnected || updateButton.offsetParent === null) continue;

                button.value = `Updating ${i + 1}/${updateButtons.length}`;
                await Promise.resolve(updatePrice(updateButton)).catch(error => console.error("[TornBazaarFiller] Failed to update row:", error));
                await sleep(100);
            }
    ...
    function parsePricingFormula() {
        ...
        let operation = firstTagIndex === -1 ? priceDeltaRaw : priceDeltaRaw.substring(0, firstTagIndex);
        ...
        while ((match = tagPattern.exec(priceDeltaRaw)) !== null) {
            ...
            if (tagName === 'market') {
                source = 'market';
            } else if (tagName === 'combined' || tagName === 'bazaar' || tagName === 'itemmarket') {
                source = tagName;
    ...
    function normalizeItemMarketListings(data, itemId) {
        ...
        if (!data.itemmarket || !Array.isArray(data.itemmarket.listings)) {
            console.warn("[TornBazaarFiller] Item market listings unavailable:", data);
            return [];
        }

        if (data.itemmarket.item && data.itemmarket.item.id != itemId) {
            console.warn("[TornBazaarFiller] Item market API returned a different item:", data.itemmarket.item);
        }
    ...
    function getListingsForPricing(itemId, source) {
        if (source === 'itemmarket') {
            let itemMarketRequest = fetchJson(marketUrl
                                              .replace("{itemId}", itemId)
                                              .replace("{apiKey}", apiKey));
            return itemMarketRequest.then(data => normalizeItemMarketListings(data, itemId));
        }

        if (source === 'bazaar') {
            let bazaarRequest = fetchJson(weav3rMarketplaceUrl.replace("{itemId}", itemId));
        ...
        return Promise.allSettled([itemMarketRequest, bazaarRequest]).then(results => {
            let listings = [];

            if (results[0].status === 'fulfilled') {
                listings = listings.concat(normalizeItemMarketListings(results[0].value, itemId));
            } else {
                console.warn("[TornBazaarFiller] Item market request failed:", results[0].reason);
            }

            if (results[1].status === 'fulfilled') {
                listings = listings.concat(normalizeBazaarListings(results[1].value));
            } else {
                console.warn("[TornBazaarFiller] Bazaar request failed, using item market only:", results[1].reason);
            }
    ...
    function selectClusterAwareListing(listings, slotOffset) {
        ...
        for (let listing of listings) {
            ...
            if (cluster) {
                cluster.count++;
                cluster.quantity += listing.quantity || 1;
                continue;
            }
    ...
    function calculatePrice(itemId) {
        ...
        if (formula.source === 'market') {
            ...
            return fetchJson(requestUrl).then(data => {
                validateTornApiResponse(data);
                let price = Number(data.items && data.items[itemId] && data.items[itemId].market_value);
    ...
    function applyPriceComparison(result, percentageOverlaySpan, wave) {
        ...
        let priceCoefficient = ((result.lowBallPrice / result.comparisonPrice) * 100).toFixed(0);
